...
const sortyByTime: ArticleSortOrderFn = ( direction: number ) => ( a: Article, b: Article ) => {
...
export class ArticleService {
  ...
  private _sortyByDirectionSubject: BehaviorSubject<number> = new BehaviorSubject<number>( 1 );
  private _sortyBySubject: BehaviorSubject<ArticleSortOrderFn> =
    new BehaviorSubject<ArticleSortOrderFn>( sortyByTime );
  ...
  constructor( private http: Http ) {
    ...
    this.orderedArticles = Observable.combineLatest(
      this._articles,
      this._sortyBySubject,
      this._sortyByDirectionSubject,
  ...
  private _makeHttpRequest( path: string,
                            sourceKey?: string ): Observable<any> {
